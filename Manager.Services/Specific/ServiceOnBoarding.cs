...
namespace Manager.Services.Specific
{
  public class ServiceOnBoarding : Repository<OnBoarding>, IServiceOnBoarding
  {
    ...
    public ViewListOnBoarding New(string idperson, string plataform)
    {
      try
      {
        ...
        if (onBoarding == null)
        {
          ...
          Task.Run(() => LogSave(_user._idPerson, string.Format("Start process | {0}", onBoarding._id), plataform));
        }
        else
        {
          if (onBoarding.StatusOnBoarding == EnumStatusOnBoarding.WaitPerson)
          {
            ...
            Task.Run(() => LogSave(_user._idPerson, string.Format("Send person approval | {0}", onBoarding._id), plataform));
    ...
    public string DeleteComments(string idonboarding, string iditem, string idcomments, string plataform)
    {
      try
      {

        var onboarding = serviceOnboarding.GetAllNewVersion(p => p._id == idonboarding).Result.FirstOrDefault();
        Task.Run(() => LogSave(_user._idPerson, string.Format("Delete comment | {0} item {1} comment {2}", idonboarding, iditem, idcomments), plataform));
        foreach (var item in onboarding.Activities)
        {
          if (item._id == iditem)
          {
    ...
    public string Delete(string id, string plataform)
    {
      try
      {

        var onboarding = serviceOnboarding.GetNewVersion(p => p._id == id).Result;
        Task.Run(() => LogSave(_user._idPerson, string.Format("Delete | {0}", id), plataform));
    ...
    public List<ViewListOnBoarding> ListEnded(string idmanager, string plataform, ref long total, string filter, int count, int page)
    {
      try
      {
        Task.Run(() => LogSave(_user._idPerson, "List ended", plataform));
        int skip = (count * (page - 1));
    ...
    public string AddCommentsSpeech(string idonboarding, string iditem, string link, EnumUserComment user, string totalimte, string plataform)
    {
      try
      {
        ...
        AddComments(idonboarding, iditem, view, plataform);
        return "ok";
      }
      catch (Exception e)
      {
    ...
    public List<ViewCrudComment> AddComments(string idonboarding, string iditem, ViewCrudComment comments, string plataform)
    {
      try
      {

        var onboarding = serviceOnboarding.GetNewVersion(p => p._id == idonboarding).Result;
        Task.Run(() => LogSave(_user._idPerson, string.Format("Add comment | {0} item {1}", idonboarding, iditem), plataform));
        foreach (var item in onboarding.Activities)
        {
          if (item._id == iditem)
          {
    ...
    public List<ViewListOnBoarding> ListPersonEnd(string idmanager, string plataform, ref long total, string filter, int count, int page)
    {
      try
      {
        Task.Run(() => LogSave(_user._idPerson, "List ended for person", plataform));
        int skip = (count * (page - 1));
    ...
    public string Update(ViewCrudOnboarding view, string plataform)
    {
      try
      {
        ...
        if (person.User._id != _user._idUser)
        {
          ...
          if (onboarding.StatusOnBoarding == EnumStatusOnBoarding.End)
          {
            ...
            Task.Run(() => LogSave(_user._idPerson, string.Format("Conclusion process | {0}", onboarding._id), plataform));
          }
        }
        else
        {
          if (onboarding.StatusOnBoarding == EnumStatusOnBoarding.End)
          {
            if (ValidOnboardingComments(onboarding))
            {
              ...
              Task.Run(() => LogSave(_user._idPerson, string.Format("Send manager review | {0}", onboarding._id), plataform));
            }
            else
            {
              if (onboarding.Person.TypeJourney == EnumTypeJourney.OnBoardingOccupation)
          ...
          else if (onboarding.StatusOnBoarding == EnumStatusOnBoarding.WaitManager)
          {
            ...
            Task.Run(() => LogSave(_user._idPerson, string.Format("Send manager approval | {0}", onboarding._id), plataform));
          }
    ...
    public string UpdateStatusOnBoarding(string idonboarding, EnumStatusOnBoarding status, string plataform)
    {
      try
      {
        var onboarding = serviceOnboarding.GetNewVersion(p => p._id == idonboarding).Result;
    ...
    private void LogSave(string idperson, string local, string plataform)
    {
      try
      {
        ...
        var log = new ViewLog()
        {
          Description = "OnBoarding " + plataform,
          Local = local,
          _idPerson = user._id
        };
