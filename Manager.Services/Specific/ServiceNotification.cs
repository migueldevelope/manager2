  ...
  public class ServiceNotification : Repository<ConfigurationNotifications>, IServiceNotification
  {
    ...
    public async void MailPlanSeq1(Person person, double totaldays)
    {
      try
      {
        ...
        var url = "";
        var body = model.Message.Replace("{Person}", person.Name).Replace("{Link}", model.Link).Replace("{Manager}", person.Manager.Name).Replace("{Days}", Math.Truncate(totaldays).ToString());
