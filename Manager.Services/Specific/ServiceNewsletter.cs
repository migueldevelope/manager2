  ...
  public class ServiceNewsletter : Repository<Newsletter>, IServiceNewsletter
  {
    ...
    public List<ViewListNewsletter> ListNewsletter(EnumPortal portal, ref long total, int count = 10, int page = 1, string filter = "")
    {
      try
      {
        ...
        && p.Enabled == true && p.Title.ToUpper().Contains(filter.ToUpper())).Result;
        ...
        total = detail.Count();
        return detail.OrderByDescending(p => p.BeginDate).Skip(skip).Take(count).Select(x => x.GetViewList()).ToList();
      }
      catch (Exception e)
      {
        throw e;
    ...
    public List<ViewListNewsletter> ListNewsletterOld(EnumPortal portal, ref long total, int count = 10, int page = 1, string filter = "")
    {
      try
      {
        var datenow = DateTime.Parse(DateTime.Now.Year + "-" + DateTime.Now.Month + "-" + DateTime.Now.Day + " 00:00");
